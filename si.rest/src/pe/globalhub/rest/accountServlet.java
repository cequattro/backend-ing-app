...
public class accountServlet extends HttpServlet {
	...
	protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        //
        List<account> reporte=new ArrayList<account>();
        Vector alumnos = Sql.consulta("SELECT a.accointid, a.iban, a.overallbalance, a.producttype, a.currency FROM `grantia`.`customer` c, `grantia`.`account` a, `grantia`.`customacc` ca"+
        		" where c.UUIDCustomer=ca.UUIDCustomer and a.accointid=ca.accointid and c.UUIDCustomer="+customer);//Con DB2
		...
		reporte.add(new account(Integer.parseInt(unaFila[0].toString()), unaFila[1].toString(), Double.parseDouble(unaFila[2].toString()), unaFila[3].toString(), unaFila[4].toString()));
